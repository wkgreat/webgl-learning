import { WebGPUContext } from "./webgpuUtils";
...
export function createTextureFromImage(
    ...
    flipY: boolean = false,
    mimmap: boolean = false): GPUTexture | null {
    return createCubeMapTexture(device, [source], flipY, mimmap);
}
...
export async function createCubeMapFromImages(urls: string[]): Promise<ImageBitmap[]> {
    return await Promise.all(urls.map(url => loadImageBitmap(url)));
}

export function createCubeMapTexture(device: GPUDevice, sources: GPUCopyExternalImageSource[], flipY: boolean = false, mimmap: boolean = false): GPUTexture | null {
    ...
    if (mimmap) {
        genMimMapForTexture(device, texture);
    }
...
*/
export function genMimMapForTexture(device: GPUDevice, texture: GPUTexture) {
