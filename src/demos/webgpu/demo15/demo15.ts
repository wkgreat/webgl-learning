...
import { createCubeMapFromImages, createCubeMapTexture, generateCubeMapFaces } from './texture';
...
async function main() {
    ...
    const faces = await createCubeMapFromImages([
        'assets/data/Leadenhall_Market/pos-x.jpg',
        'assets/data/Leadenhall_Market/neg-x.jpg',
        'assets/data/Leadenhall_Market/pos-y.jpg',
        'assets/data/Leadenhall_Market/neg-y.jpg',
        'assets/data/Leadenhall_Market/pos-z.jpg',
        'assets/data/Leadenhall_Market/neg-z.jpg',
    ]);
    ...
    const module = device.createShaderModule({
        ...
        @vertex fn vs(vert: Vertex) -> VsOutput {
            ...
            let world_pos = matrices.worldmtx * vert.position;
            let norm = normalize((matrices.worldmtx * vec4f(vert.normal, 1.0)).xyz);
            let camera_world_pos = camera.eye;
            let view = normalize(world_pos.xyz - camera_world_pos);
            let refv = reflect(view, norm);

            output.position = matrices.projmtx * matrices.viewmtx * world_pos;
            output.normal = norm;
        ...
        @fragment fn fs(input: VsOutput) -> @location(0) vec4f {
            ...
            return textureSample(theTexture, theSampler, input.refv);
        }
        `
    });
    ...
    const cameraUniform = device.createBuffer({
        label: `${name} camera uniform buffer`,
        size:
            4 * 3 + vec3fpad +
            4 * 3 + vec3fpad +
            4 * 3 + vec3fpad,
    ...
    const cameraData = new Float32Array(3 * 3);
    ...
    let deltaTime = 0;
    let cameraRotation = {
        dx: 0.005 * 180 / Math.PI,
        dy: 0.01 * 180 / Math.PI,
        dz: 0.015 * 180 / Math.PI,
        x: 0,
        y: 0,
        z: 0
    }
    ...
    function render(time: number) {
        ...
        deltaTime = (time - then) / 1000;
        then = time;
        cameraRotation.x = (cameraRotation.x + deltaTime * cameraRotation.dx) % (Math.PI * 2);
        cameraRotation.y = (cameraRotation.y + deltaTime * cameraRotation.dy) % (Math.PI * 2);
        cameraRotation.z = (cameraRotation.z + deltaTime * cameraRotation.dz) % (Math.PI * 2);
        const cameraRotMtx = mat4.create();
        mat4.rotateX(cameraRotMtx, cameraRotMtx, cameraRotation.x);
        mat4.rotateY(cameraRotMtx, cameraRotMtx, cameraRotation.y);
        mat4.rotateZ(cameraRotMtx, cameraRotMtx, cameraRotation.z);

        const cameraFrom4 = vec4.fromValues(2, 2, 2, 1);
        vec4.transformMat4(cameraFrom4, cameraFrom4, cameraRotMtx);
        const cameraFrom = (cameraFrom4 as Float32Array).slice(0, 3);
        const cameraTo = new Float32Array([0, 0, 0]);
        ...
        if (cubemapTexture !== null) {
            ...
            const bindGroup = device.createBindGroup({
                ...
                entries: [
                    ...
                    { binding: 3, resource: { buffer: cameraUniform } }
                ]
            });
            ...
            const viewmtx = mat4.lookAt(mat4.create(), cameraFrom, cameraTo, cameraUp);
            ...
            cameraData.set(cameraFrom, 0);
            cameraData.set(cameraTo, 3);
            cameraData.set(cameraUp, 6);
            device.queue.writeBuffer(cameraUniform, 0, cameraData);
